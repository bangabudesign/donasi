...
@section('content')
<!-- select amount -->
<form action="{{ route('zakat.store', $zakat->id) }}" method="post">
    @csrf
    ...
    <div class="container max-w-md mx-auto px-4 pt-20 pb-24 min-h-screen bg-white">
        <div class="mb-4">
            <label class="form-label uppercase">Isi nominal {{ $zakat->name }}L</label>
            <div class="relative">
                <span class="text-xl font-bold text-gray-700 absolute top-0 py-2 px-3">Rp</span>
                <input type="number" placeholder="0" name="amount" class="block form-control mt-3 text-right text-xl font-bold @error('amount') invalid @enderror" value="{{ old('amount') }}">
            </div>
            @error('amount')
...
<script>
    const paymentUrl = "{{ route('zakat.payment', $zakat->id) }}";
    const contributeUrl = "{{ route('zakat.contribute', $zakat->id) }}";
</script>
